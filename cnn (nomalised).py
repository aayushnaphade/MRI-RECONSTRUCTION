...
from tensorflow.keras.layers import Input, Conv2D, Lambda, Conv2DTranspose, concatenate, BatchNormalization, Activation, Add
...
def load_and_preprocess_data(file_names, data_path):
    ...
    for file_name in file_names:
        ...
        # Normalize the magnitude data to the range [0, 1]
        magnitude_data /= np.max(magnitude_data)
...
def create_cnn_model(input_shape):
    ...
    real_part, imag_part = Lambda(lambda x: (tf.math.real(x), tf.math.imag(x)))(inputs)

    # Real-valued convolution layers with BatchNormalization and ReLU activation
    conv1_real = Conv2D(32, (3, 3), padding='same')(real_part)
    conv1_real = BatchNormalization()(conv1_real)
    conv1_real = Activation('relu')(conv1_real)
    
    conv2_real = Conv2D(64, (3, 3), padding='same')(conv1_real)
    conv2_real = BatchNormalization()(conv2_real)
    conv2_real = Activation('relu')(conv2_real)

    # Imaginary-valued convolution layers with BatchNormalization and ReLU activation
    conv1_imag = Conv2D(32, (3, 3), padding='same')(imag_part)
    conv1_imag = BatchNormalization()(conv1_imag)
    conv1_imag = Activation('relu')(conv1_imag)
    
    conv2_imag = Conv2D(64, (3, 3), padding='same')(conv1_imag)
    conv2_imag = BatchNormalization()(conv2_imag)
    conv2_imag = Activation('relu')(conv2_imag)
    ...
    deconv1_real = Conv2DTranspose(32, (3, 3), padding='same')(merged_conv)
    deconv1_real = BatchNormalization()(deconv1_real)
    deconv1_real = Activation('relu')(deconv1_real)

    # Complex-valued output layer
    outputs = Conv2D(1, (3, 3), padding='same', activation='linear')(deconv1_real)

    # Merge real and imaginary parts back to complex-valued data
    outputs_complex = Lambda(lambda x: tf.complex(x[0], x[1]))([outputs, tf.zeros_like(outputs)])
...
history = cnn_model.fit(x_train_undersampled, x_train, batch_size=16, epochs=1, validation_data=(x_val, x_val))
...
for i in range(x_test.shape[0]):
    ...
    # Calculate SNR
    signal_power = np.sum(np.abs(original_image) ** 2)
    noise_power = np.sum(np.abs(original_image - reconstructed_image) ** 2)
    snr = 10 * np.log10(signal_power / noise_power)
    snr_values.append(snr)
    
    # Calculate RMSE
    rmse = np.sqrt(np.mean((np.abs(original_image) - np.abs(reconstructed_image)) ** 2))
